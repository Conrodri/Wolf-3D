...
/*                                                 #+#   #+    #+    #+#      */
/*   Created: 2020/01/27 16:51:31 by conrodri     #+#   ##    ##    #+#       */
/*   Updated: 2020/01/28 12:34:59 by qpupier     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void	init_check_so(t_param *p, int i)
{
	p->dx = p->rays[i].a;
	...
	while (!p->stop_x && p->tabx--)
		if (p->rays[i].a * (p->tabx + 1) + p->rays[i].c >= p->sizey)
			p->stop_x = 2;
...
void		check_so(t_param *p, int i)
{
	...
	while (!p->stop_y && ++p->taby < p->sizey)
		if ((p->taby - p->rays[i].c) / p->rays[i].a < 0)
			p->stop_y = 2;
