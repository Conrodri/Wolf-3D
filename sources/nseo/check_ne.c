...
/*                                                 #+#   #+    #+    #+#      */
/*   Created: 2020/01/27 16:51:28 by conrodri     #+#   ##    ##    #+#       */
/*   Updated: 2020/01/29 15:54:06 by qpupier     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void	init_check_ne(t_param *p, int i)
{
	p->dx = p->rays[i].a;
	...
	while (!p->stop_x && ++p->tabx < p->sizex)
		if (p->rays[i].a * p->tabx + p->rays[i].c < 0)
			p->stop_x = 2;
...
void		check_ne(t_param *p, int i)
{
	...
	{
		if ((p->taby + 1 - p->rays[i].c) / p->rays[i].a >= p->sizex)
			p->stop_y = 2;
